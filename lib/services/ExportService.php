...
// Serviciu pentru exportul documentelor si datelor generate
...
class ExportService
{
    ...
    // Exporta un document existent in formatul specificat
    public function exportDocument(int $documentId, int $userId, string $format): array
    {
        ...
        switch ($format) {
            case 'html': $result = $this->exportAsHtml($document); break;
            case 'pdf':  $result = $this->exportAsPdf($document);  break;
            case 'csv':  $result = $this->exportAsCsv($document);  break;
            case 'json': $result = $this->exportAsJson($document);  break;
        }
    ...
    // Genereaza date dupa schema primita si le exporta in CSV sau JSON
    public function exportData(array $fields, int $rows, string $format, int $userId): array
    {
        ...
        // Construim randurile cu labeluri ca chei, pentru export
        $exportRows = [];
    ...
    // Incarca un document din baza de date dupa ID si utilizator
    ...
    // Exporta documentul ca fisier HTML stilizat
    private function exportAsHtml(array $document): array
    {
        ...
        // Adaugam stilizare daca fisierul nu contine deja un document HTML complet
...
<div class="doc-footer">Document generat automat &bull; ' . date('d.m.Y H:i') . '</div>
    ...
    // Exporta documentul ca fisier PDF via PdfExporter
    ...
    // Exporta datele documentului ca fisier CSV
    ...
    // Exporta metadatele si campurile documentului ca fisier JSON
    ...
    // Exporta randuri generate ca fisier CSV si logheaza actiunea
    ...
    // Exporta randuri generate ca fisier JSON si logheaza actiunea
    ...
    // Obtine campurile unui document din schema sau sablon asociat
    private function getDocumentFields(array $document): array
    {
        // Prioritate: schema salvata > sablon > array gol
        ...
        if (!empty($document['template_id'])) {
            ...
            if ($template) {
                ...
                // Sablon HTML custom - extragem variabilele si le mapam la tipuri
                $typeMap = [
                    'nume'           => 'full_name', 'email'        => 'email',
                    'telefon'        => 'phone',     'adresa'       => 'address',
                    'data_nasterii'  => 'date',      'cnp'          => 'cnp',
                    'ocupatie'       => 'job_title', 'studii'       => 'education',
                    'firma'          => 'company',   'nr_factura'   => 'invoice_number',
                    'cui'            => 'cui',       'iban'         => 'iban',
                    'pret'           => 'price',     'tva'          => 'tva',
                    'data'           => 'date',      'oras'         => 'city',
                    'judet'          => 'county',    'produs'       => 'product',
                    'suma'           => 'price',     'cantitate'    => 'number',
                    'pret_unitar'    => 'price',     'nume_solicitant' => 'full_name',
                    'detalii'        => 'paragraph', 'subiect'      => 'text',
                    'data_emitere'   => 'date',      'furnizor'     => 'company',
                    'cui_furnizor'   => 'cui',       'client'       => 'company',
                    'cui_client'     => 'cui'
                ];
    ...
    // Extrage valorile campurilor dintr-un fisier HTML generat
    private function extractDataFromHtml(string $html, array $fields): array
    {
        ...
        foreach ($fields as $field) {
            ...
            // Caz 1: element cu atributul data-field="cheie"
            ...
            // Caz 2: paragraf generat ca <p><strong>Label:</strong> valoare</p>
            $patternLabel = '/<strong>\s*' . preg_quote($label, '/') . '\s*:?\s*<\/strong>\s*(.*?)\s*<\/p>/is';
            if (preg_match($patternLabel, $html, $matches)) {
                $data[$key] = html_entity_decode(trim(strip_tags($matches[1])), ENT_QUOTES | ENT_HTML5, 'UTF-8');
                continue;
            }
    ...
    // Inregistreaza exportul in tabela exports si in loguri
